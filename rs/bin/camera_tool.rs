...
fn main() {
    ...
    opts.optopt("f", "focus_at", "Focus the camera at a given distance", "DISTANCE");
    opts.optopt("o", "out", "(Optional) where to save the changed configuration", "FILE");
    ...
    let mut camera = CameraConfig::<f32>::from_map(
        &table_from_file(matches.opt_str("camera").unwrap()).expect("Error reading config")
    ...
    if matches.opt_present("focus_at") {
        let focus_distance: f32 = matches.opt_str("focus_at").unwrap().parse().expect("Error parsing focus distance");
