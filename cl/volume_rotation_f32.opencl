...
kernel void rotate_filter_z(
    ...
    const float src_fz1 = ceil(LightVolume_z2iz(geom, z + my_kernel->tau3));
    ...
    for(int src_iz = src_iz0; src_iz < src_iz1; ++src_iz) {
        ...
        //accum += TrapezoidSplineKernel_integrate(my_kernel, z, src_z0, src_z1) * input[ix + geom->nx*(iy + geom->ny*src_iz)];
    }
...
kernel void rotate_filter_y(
    ...
    const float src_fy1 = ceil(LightVolume_y2iy(geom, y + my_kernel->tau3));
...
kernel void rotate_filter_x(
    ...
    global struct TrapezoidSplineKernel* my_kernel = kernels + iy + geom->ny*iz;

    const float src_fx0 = floor(LightVolume_x2ix(geom, x + my_kernel->tau0));
    const float src_fx1 = ceil(LightVolume_x2ix(geom, x + my_kernel->tau3));
